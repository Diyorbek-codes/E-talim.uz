...
//            ->where('fan_uqituvchis.uqituvchi_id', auth()->user()->uqituvchi->id)
        ...
        return view('ohtm_uquv_bulimi.videmost', compact('fannomi','videmost','harbiy_unvon','nazoratchi' ,'oraliq_uqituvchi','joriy_uqituvchi'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($jami,$data, $request, $id) {
            ...
            if($fan->save()){
                $videmost = $fan->save();
                foreach ($data['kursants'] as $kur){
                    $videmostBaho = VidemostBaho::where(['videmost_id'=>$id, 'tinglovchi_id'=>$kur->id])->first();
                    $videmostBaho->update([
                        'joriy'=>$kur->joriy_bahos,
                        'oraliq'=>$kur->oraliq_bahos,
                        'yakuniy'=>$kur->yakuniy_bahos,
