...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <h4 class="mb-0">O'qituvchilar</h4>
    ...
    <div class="row">
        <div class="card col-lg-12">
            <div class="card-body">
                <h4 class="card-title mb-4 font-size-22 text-center">Kafedra o'qituvchilari</h4>
                <div class="table-responsive ">
                    <table class="table table-centered datatable dt-responsive nowrap text-center" data-page-length="5"
                        ...
                        <tr>
                            ...
                            <th><strong>Ilmiy daraja</strong></th>
                            <th><strong>Mutaxassisligi</strong></th>
                            <th><strong>Kafedralar</strong></th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach ($uqituvchi as $uqituv)
                            <tr href="{{route('uquvbulumi.kaf_uqituvchi.index', $uqituv->id) }}">
                                <td>{{ $loop->iteration }}</td>
                                <td><a href="{{route('uquvbulumi.uqituvchi_yuklama.index', $uqituv->id)}}"><strong>{{$uqituv->fio}}</strong></a></td>
                                <td>{{$uqituv->unvon_nomi}}</td>
                                <td>{{$uqituv->tugilgan_sana}}</td>
                                <td>{{$uqituv->ilmiy_unvon_nomi}}</td>
                                <td>{{$uqituv->ilmiy_daraja_nomi}}</td>
                                <td>{{$uqituv->mutaxassisligi}}</td>
                                <td>{{$uqituv->kaf_nomi}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
